%!TEX root = ../template.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Config/7_package.tex
%% NOVA thesis configuration file
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\typeout{NT FILE Config/5_packages.tex}%

% \ntlangsetup{pt/contentsname=O MEU ÍNDICE}
% \ntlangsetup{en/contentsname=MY LITTLE TOC}

%%============================================================
%%  FOR DEMO PURPOSES ONLY --- YOU MAY REMOVE THESE PACKAGES
%%============================================================
%\PassOptionsToPackage[x11names]{xcolor} %package to use colored text
\usepackage{float}        % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\usepackage{wrapfig}      % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\restylefloat{figure}     % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\usepackage{metalogo}     % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\usepackage{lipsum}       % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\usepackage{byo-twemojis}     % to typeset emojis
\newcommand{\emojiSmile}{\raisebox{-3pt}{\scalebox{1.3}{\byoTwemoji{head; eyes normal opened; mouth laughing}}}}

\usepackage[acronym]{glossaries}
%\usepackage{glossaries-extra}
%============================================================
%%  ADD YOUR PACKAGES AND OTHER CUSTOMIZATIONS HERE
%%============================================================

% \usepackage{widows-and-orphans} % add warnings about widows and orphans to “template.log”
% \usepackage{refcheck}     % list used and unused labels

\usepackage{paralist}     % To enable costumizble enumerates.  See the documentation.

\usepackage[textsize=footnotesize]{todonotes}  % To register TODO notes in the text

\usepackage{makecell}      % Multilie table cells and control vertical and horizontal alignment

\usepackage{amsmath} %mathematical functions

\usepackage[export]{adjustbox} %Enable the usage of images inside a sentence, next to a word

\usepackage{graphicx} %package for image display

\usepackage{backnaur} %package for backus naur text rules

%\newfontfamily\symb{Symbola} %Package to use special symbols

\usepackage{booktabs}

\usepackage{multirow}

\usepackage{multicol}

\usepackage{tabularx}

\usepackage{cancel}

\usepackage{subcaption}

\usepackage{array}

\usepackage[T1]{fontenc}
\usepackage{courier}

\usepackage{gensymb} % for the degree symbol

\usepackage{scalerel} % stretch images

\usepackage{csquotes}

\definecolor{myblue}{RGB}{83, 119, 209}
\definecolor{myred}{RGB}{231, 95, 74}
\definecolor{mygreen}{RGB}{1, 163, 165}

\definecolor{myblue2}{RGB}{47, 62, 101}
\definecolor{myorange}{RGB}{255, 217, 40}
\definecolor{myorange2}{RGB}{237, 162, 100}


\definecolor{mygreen2}{RGB}{31, 177, 177}
\definecolor{mypurple}{RGB}{203, 81, 171}
\definecolor{myblue3}{RGB}{51, 194, 223}
\definecolor{myblue4}{RGB}{72, 129, 214}

\definecolor{mygreen3}{RGB}{49, 216, 202}
\definecolor{myblue5}{RGB}{7, 60, 159}
\definecolor{mymagenta}{RGB}{216, 49, 136}

\DeclareMathOperator{\sign}{sign}

\newcolumntype{A}{p{0.05\textwidth}}
\newcolumntype{B}{p{0.5\textwidth}}
\newcolumntype{C}{p{0.325\textwidth}}
\newcolumntype{E}{p{0.5\textwidth}}
\newcolumntype{P}{p{0.6\textwidth}}
\newcolumntype{L}[1]{>{\raggedright\let\newline\\\arraybackslash\hspace{0pt}}m{#1}}
\newcolumntype{M}[1]{>{\centering\arraybackslash}m{#1}}
\newcolumntype{?}{!{\vrule width 1pt}}
\newcolumntype{/}{!{\vrule width 0.5pt}}

\usepackage{longtable}
\usepackage{lscape}
\usepackage{collcell}
\usepackage{xfp}
\usepackage{pgf}

% This selects the amount of color to use
\newcommand*{\MinNumber}{0}%
\newcommand*{\MaxNumber}{1}%

%new math operators
\DeclareMathOperator{\SSM}{SSM}

\newcommand{\ApplyGradient}[1]{%
\ifnum\fpeval{#1 > 0} = 1
	\pgfmathsetmacro{\PercentColor}{100.0*(#1-\MinNumber)/(\MaxNumber-\MinNumber)}
    \textcolor{black!\PercentColor}{#1}
\fi
}

\newcommand{\maxf}[1]{{\cellcolor[gray]{0.8}} #1}


\newcolumntype{R}{>{\collectcell\ApplyGradient}{r}<{\endcollectcell}}

% \usepackage{indentfirst}       % Indent first list after a (sub)section title

% \usepackage{algorithm2e}
% \RestyleAlgo{ruled}

% \usepackage{minted}    % Fontification of source code listings

% \usepackage{listings}    % Fontification of source code listingsConfig/5_packages.tex
%                          % In alternative, use the package “minted” (highly recomended)

% \lstset{
%     captionpos=t,
%     basicstyle={\ttfamily\footnotesize},
%     numbers=left,
%     numberstyle={\ttfamily\tiny},
%     tabsize=2,
%     language=Java,
%     float,
%     frame=single,
%     columns=fullflexible,
%     breaklines=true,
%     postbreak=\mbox{\textcolor{red}{$\hookrightarrow$}\space},
%     inputencoding=utf8,
%     extendedchars=true,
%     literate=
%       {á}{{\'a}}1 {é}{{\'e}}1 {í}{{\'i}}1 {ó}{{\'o}}1 {ú}{{\'u}}1
%       {Á}{{\'A}}1 {É}{{\'E}}1 {Í}{{\'I}}1 {Ó}{{\'O}}1 {Ú}{{\'U}}1
%       {à}{{\`a}}1 {è}{{\`e}}1 {ì}{{\`i}}1 {ò}{{\`o}}1 {ù}{{\`u}}1
%       {À}{{\`A}}1 {È}{{\'E}}1 {Ì}{{\`I}}1 {Ò}{{\`O}}1 {Ù}{{\`U}}1
%       {ä}{{\"a}}1 {ë}{{\"e}}1 {ï}{{\"i}}1 {ö}{{\"o}}1 {ü}{{\"u}}1
%       {Ä}{{\"A}}1 {Ë}{{\"E}}1 {Ï}{{\"I}}1 {Ö}{{\"O}}1 {Ü}{{\"U}}1
%       {â}{{\^a}}1 {ê}{{\^e}}1 {î}{{\^i}}1 {ô}{{\^o}}1 {û}{{\^u}}1
%       {Â}{{\^A}}1 {Ê}{{\^E}}1 {Î}{{\^I}}1 {Ô}{{\^O}}1 {Û}{{\^U}}1
%       {œ}{{\oe}}1 {Œ}{{\OE}}1 {æ}{{\ae}}1 {Æ}{{\AE}}1 {ß}{{\ss}}1
%       {ç}{{\c c}}1 {Ç}{{\c C}}1 {ø}{{\o}}1 {å}{{\r a}}1 {Å}{{\r A}}1
%       {€}{{\EUR}}1 {£}{{\pounds}}1
% }

